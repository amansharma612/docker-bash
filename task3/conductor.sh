...
run()
{
    ...
    #mkdir $CONTAINERDIR/$NAME/rootfs/proc/
    unshare --fork -p -i -u -n -m --mount-proc --kill-child --root=$CONTAINERDIR/$NAME/rootfs bash -c "mount -t sysfs sysfs /sys; chmod -R 755 /dev/; $INIT_CMD_ARGS;"
}
...
stop()
{
    ...
    # umount "$CONTAINERDIR/$NAME/ns/ipc" > /dev/null 2>&1 || :
    # umount "$CONTAINERDIR/$NAME/ns/uts" > /dev/null 2>&1 || :
    # umount "$CONTAINERDIR/$NAME/ns/pid" > /dev/null 2>&1 || :
    # umount "$CONTAINERDIR/$NAME/ns/net" > /dev/null 2>&1 || :
    # umount "$CONTAINERDIR/$NAME/ns/mount" > /dev/null 2>&1 || :
...
exec()
{
    ...
    # ps -ef | grep "$CONTAINERDIR/$NAME/rootfs" | grep -v grep
    nsenter -t $CONTAINER_INIT_PID -p -n -u -i chroot $CONTAINERDIR/$NAME/rootfs bash -c "mount -t proc proc /proc; $EXEC_CMD_ARGS"
...
addnetwork()
{
    ...
    ip addr add $OUTSIDE_IP4 dev $OUTSIDE_PEER
    ip netns exec $NAME ip addr add $INSIDE_IP4 dev $INSIDE_PEER
