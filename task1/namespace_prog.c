...
int main() {
    ...
    */
    ...
    sprintf(path1, "/proc/%d/ns/pid", child_pid);
    sprintf(path2, "/proc/%d/ns/uts", child_pid);

    int pid_fd = open(path1, O_RDONLY);
    int uts_fd = open(path2, O_RDONLY);
    ...
    if (fork() == 0) {
        ...
        */
        ...
        // setns(fd, CLONE_NEWUTS | CLONE_NEWPID);
        setns(uts_fd, 0);
        setns(pid_fd, 0);
